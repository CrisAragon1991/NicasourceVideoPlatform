...
export class CreateVideoImplementation implements ICreateVideoUseCase {
    ...
    async execute (video: VideoRegisterDto, file: UploadedFile, userId: number): Promise<Video> {
        ...
        const types = ['webm', 'WEBM', 'mp4', 'MP4', 'avi', 'AVI', 'wmv', 'WMV']
        ...
        const filename = `${filenameSplited[0]}${Date.now()}.${filenameSplited[filenameSplited.length - 1]}`
        const pathCurrentVideo = `${VIDEO_PATH}${path.sep}${filename}`
